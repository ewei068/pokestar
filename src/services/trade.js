...
const { MAX_TRADE_POKEMON, MAX_TRADE_MONEY, TRADE_COMPLETE_WINDOW, TRADE_ACCEPT_WINDOW } = require("../config/socialConfig");
...
const canTrade = async (trainer) => {
    ...
    if (trainer.level < levelReq) {
        return { data: false, err: `<@${trainer.userId}>: You must be level ${levelReq} to trade.` };
    }
    return { err: null };
}
...
const getTradePokemons = async (trainer, trade=null) => {
    ...
    if (canReleaseRes.err) {
        return { data: null, err: `<@${trainer.userId}>: ${canReleaseRes.err} Use \`/traderemove ALL\` to fix trade.` };
    }
    ...
    if (pokemons.data.length !== pokemonIds.length) {
        return { data: null, err: `<@${trainer.userId}>: Error finding trade pokemon. Use \`/traderemove ALL\` to fix trade.` };
    }

    // if dupes, return
    const pokemonIdsSet = new Set(pokemonIds);
    if (pokemonIdsSet.size !== pokemonIds.length) {
        return { data: null, err: `<@${trainer.userId}>: Duplicate Pokemon in trade. Use \`/traderemove ALL\` to fix trade.` };
    }
...
const validateTrade = async (trainer, trade) => {
    ...
    if (tradePokemons.data.length < 0 || tradePokemons.data.length > MAX_TRADE_POKEMON) {
        return { err: `<@${trainer.userId}>: You can only trade between 0 and ${MAX_TRADE_POKEMON} pokemon at a time.` };
    }

    // make sure money between 0 and max trade money
    if (trade.money < 0 || trade.money > MAX_TRADE_MONEY) {
        return { err: `<@${trainer.userId}>: You can only trade between 0 and ${MAX_TRADE_MONEY} money at a time.` };
    }

    // make sure trainer has enough money
    if (trade.money > trainer.money) {
        return { err: `<@${trainer.userId}>: You do not have enough money to trade ${formatMoney(trade.money)}.` };
    }
...
const buildTradeRequestSend = async ({stateId=null, user1=null, userId2=null}={}) => {
    ...
    const acceptButton = buildButtonActionRow(buttonConfigs, eventNames.TRADE_REQUEST_ACCEPT);
...
const onTradeRequestAccept = async ({stateId=null, user2=null}={}) => {
    ...
    const trade = trainer2.data.trade;

    // validate both trades
    const validate = await validateTrades(trainer1.data, trainer2.data, state.trade1, trade);
    if (validate.err) {
...
const onTradeAccept = async ({stateId=null, user=null}={}) => {
    ...
    if (Date.now() - state.tradeStartedTime < TRADE_ACCEPT_WINDOW * 1000) {
        return { err: `You cannot accept a trade within ${TRADE_ACCEPT_WINDOW} seconds of it starting.` };
    }
    ...
    if (state.user1Accepted && state.user2Accepted && state.tradeAcceptedTime === undefined) {
...
const onTradeComplete = async ({stateId=null}={}) => {
    ...
    const trade1 = state.trade1;
    const trade2 = state.trade2;

    // validate trades
    const validate = await validateTrades(trainer1.data, trainer2.data, trade1, trade2);
    if (validate.err) {
        return { err: validate.err };
    }
...
const buildTradeSend = async ({stateId=null}={}) => {
    ...
    const buttonConfigs = [
        {
            ...
            disabled: state.user1Accepted !== undefined || state.declineUserId !== undefined,
            data: {
                stateId: stateId,
        ...
        },
        {
            ...
            disabled: state.user2Accepted !== undefined || state.declineUserId !== undefined,
            data: {
                stateId: stateId,
    ...
    const buttons = buildButtonActionRow(buttonConfigs, eventNames.TRADE_REQUEST_BUTTON);
    ...
    if (state.declineUserId) {
        ...
        content = `Trade has been **accepted** by both parties and will complete <t:${Math.floor(state.tradeAcceptedTime / 1000) + TRADE_COMPLETE_WINDOW}:R>. **You may still cancel the trade.**`;
    }
    ...
    const send = {
        ...
        components: [...offerSend1.send.components, ...offerSend2.send.components, buttons]
    }
