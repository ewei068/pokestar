...
const buildHelpEmbed = () => {
  ...
  embed.setFooter({
    text: `If you're just starting, please take the /tutorial!`,
  });
  return embed;
};
/**
...
const buildHelpCategoryEmbed = (category) => {
  ...
  embed.setColor("#FFFFFF");
  embed.setDescription("Use the selection menu to learn more about a command!");
  embed.addFields({ name: "Commands", value: commandsString, inline: true });
  embed.setFooter({
    text: `If you're just starting, please take the /tutorial!`,
  });
  return embed;
...
const buildTutorialEmbed = (tutorialData, pageNumber) => {
  ...
  embed.setFooter({
    text: `Page ${pageNumber} | To return to this page, use /tutorial ${pageNumber}`,
  });
...
const buildHelpSend = async ({
  ...
  if (view === "help") {
    ...
    const categorySelectRow = buildIdConfigSelectRow(
      ...
      commandCategoryConfig[option].commands.filter((command) =>
