...
const { buildTradeSend, onTradeAccept, onTradeDecline, onTradeComplete } = require("../../services/trade");
...
const tradeRequestButton = async (interaction, data) => {
    ...
    } else if (interaction.user.id !== state.userId1 && interaction.user.id !== state.userId2) {
        return { err: "You cannot use this button." };
    }
    ...
    if (actionRes.shouldTrade) {
        await setTimeout(async () => {
            const { err } = await onTradeComplete({
                stateId: data.stateId,
            });
            if (err) {
                await interaction.editReply({
                    "content": `Trade could not be completed. ${err}`,
                });
            } else {
                await interaction.editReply({
