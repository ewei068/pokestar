...
const equipmentStatAggregate = function (
  equipmentLevel,
  slots,
  modifierSlotConfig,
  modifierConfig,
  ...
  for (const slotId in slots) {
    ...
    const value = Math.round(
      baseValue * (slotData.level && includeLevel ? equipmentLevel : 1)
    );
    stats[slot.modifier] = stats[slot.modifier]
      ? stats[slot.modifier] + value
      : value;
  }
  return stats;
};
