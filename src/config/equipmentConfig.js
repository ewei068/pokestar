...
const equipmentConfig = Object.freeze({
  [equipmentTypes.POWER_WEIGHT]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/8/80/Dream_Power_Weight_Sprite.png",
  ...
  [equipmentTypes.POWER_BRACER]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/1/18/Dream_Power_Bracer_Sprite.png",
  ...
  [equipmentTypes.POWER_BELT]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/9/9a/Dream_Power_Belt_Sprite.png",
  ...
  [equipmentTypes.POWER_LENS]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/a/ac/Dream_Power_Lens_Sprite.png",
    material: backpackItems.WILLPOWER_SHARD,
    slots: {
      [modifierSlots.PRIMARY]: {
  ...
  [equipmentTypes.POWER_BAND]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/9/99/Dream_Power_Band_Sprite.png",
    material: backpackItems.KNOWLEDGE_SHARD,
    slots: {
      [modifierSlots.PRIMARY]: {
  ...
  [equipmentTypes.POWER_ANKLET]: {
    ...
    sprite:
      "https://archives.bulbagarden.net/media/upload/1/1c/Dream_Power_Anklet_Sprite.png",
    material: backpackItems.EMOTION_SHARD,
    slots: {
      [modifierSlots.PRIMARY]: {
...
equipmentInfoString +=
  "Equipment is used to boost the stats of your Pokemon. Each piece of equipment has 5 stat slots: primary, secondary, and 3 substats. **Primary and secondary stats scale with the equipment level, while substats don't.**\n\n";
equipmentInfoString +=
  "Equipment stats also come with a quality, that changes the value of the stat modifier. Modifiers and stats can be changed with stat rerolls. \n\n";
equipmentInfoString +=
  "Each equipment stat modifies your Pokemon's stat in different ways. **There are three types of modifiers: Base, Percent, and Flat.** They work in the following ways:\n";
equipmentInfoString +=
  "* Base: Adds a flat amount to the original base Pokemon stat, which scales with level and nature.\n";
equipmentInfoString +=
  "* Percent: After the original stat is calculated with the traditional Pokemon stat calculation formula, increase that stat by the given percentage.\n";
equipmentInfoString +=
  "* Flat: Adds a flat amount to the final stat, after all other modifiers have been applied.\n\n";
equipmentInfoString += "**Equipment Types**\n";
equipmentInfoString +=
  "Each Pokemon equips 1 of each equipment type, which determines what stats the equipment may have. There are 6 different types of equipment, each with their own primary stat that increases the base stat. **Each type of equipment also has different possible secondary stats**, shown below:";
for (const equipmentData of Object.values(equipmentConfig)) {
  const { modifiers: equipmentModifierConfigs } =
    equipmentData.slots[modifierSlots.SECONDARY];
  const modifierNames = equipmentModifierConfigs.map(
    (modifier) => modifierConfig[modifier].name
  );
...
equipmentInfoString2 +=
  "There are two ways to modify a piece of equipment: **level upgrades and rerolling.** All modifications cost Pokedollars and Shards. Shards can be obtained from the `/dungeon`.\n\n";
equipmentInfoString2 +=
  "Level upgrades increase the level of the equipment, which increases the primary and secondary stats. The cost of upgrading increases with the level of the equipment. Equipment can be leveled to level 10.\n\n";
equipmentInfoString2 +=
  "Rerolling changes the modifiers of a given stat slot. Rerolling chooses a random (possibly duplicate) modifier as well as randomly rolls the quality of the stat.";
