...
const toggleSpawn = async (user, guildId, channelId, member) => {
  ...
  if (channelId) {
    ...
    if (guildData.spawnDisabledChannels.includes(channelId)) {
      guildData.spawnDisabledChannels = guildData.spawnDisabledChannels.filter(
        (id) => id !== channelId
      );
    } else {
    ...
    try {
      const query = new QueryBuilder(collectionNames.GUILDS)
        ...
        .setUpsert({
          $set: {
            guildId,
            spawnDisabledChannels: guildData.spawnDisabledChannels,
          },
        });
    ...
    if (guildData.spawnDisabledChannels.includes(channelId)) {
      return {
        send: `Wild Pokemon spawning **DISABLED** in <#${channelId}>. Use \`/togglespawn channel\` to revert this! Spawning is disabled in channels: <#${guildData.spawnDisabledChannels.join(
          ">, <#"
        )}>`,
      };
    }
    const channelsDisabledString =
      guildData.spawnDisabledChannels.length > 0
        ? ` Spawning is still disabled in <#${guildData.spawnDisabledChannels.join(
            ">, <#"
          )}>.`
        : "";
    return {
      send: `Wild Pokemon spawning **ENABLED** in <#${channelId}>. Use \`/togglespawn channel\` to revert this!${channelsDisabledString}`,
    };
  }
  ...
  try {
    const query = new QueryBuilder(collectionNames.GUILDS)
      .setFilter({ guildId })
      .setUpsert({
        $set: { guildId, spawnDisabled: guildData.spawnDisabled },
      });
  ...
  if (guildData.spawnDisabled) {
    return {
      send: "Wild Pokemon spawning **DISABLED.** Use `/togglespawn` to revert this! **If you wanted to disable spawns in only one channel,** re-enable spawns with `/togglespawn` and then use `/togglespawn channel`.",
    };
  }
  const channelsDisabledString =
    guildData.spawnDisabledChannels.length > 0
      ? ` Spawning is still disabled in <#${guildData.spawnDisabledChannels.join(
          ">, <#"
        )}> Use \`/togglespawn channel\` to enable spawns in a channel.`
      : "";
  return {
    send: `Wild Pokemon spawning **ENABLED.** Use \`/togglespawn\` to revert this!${channelsDisabledString}`,
  };
};
...
const toggleSpawnMessageCommand = async (message) => {
  ...
  const { send, err } = await toggleSpawn(
    message.author,
    message.guildId,
    channelId,
    message.member
